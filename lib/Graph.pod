...
Return true if the graph is regular (all vertices have the same degree).
For directed graphs, all vertices must have the same in-degree and
out-degree.
...
Return true if the graph is undirected bipartite, and all the vertices in
each partition have the same degree.
